%##############################################################
%This function is implemented for the extra credit part named Boundary Effects 
%You can directly run the file to see chopped images
%##############################################################
...
for i = 1:length(imageNames),
    ...
    %#############for Boundary effect extra credit #####################
    % chopping ugly boundary
    cutBoundary = zeros(imageHeight-60, imageWidth-60, 3);
    cutBoundary = colorIm(31:imageHeight-30,31:imageWidth-30,:); 
    %####################################################################
    ...
    imwrite(cutBoundary, outimageName);
    ...
    if display
        ...
        title('Input image');
        subplot(1,2,2); imagesc(cutBoundary); axis image off;
        title('Aligned image');
        pause(1);
    end
end
